...
import { makeProviderFromConfig } from "../jarvis";
...
function getWorkspace(config: JarvisConfig): string {
    const ws = (
        config.agents?.defaults?.workspace ?? join(homedir(), ".jarvis")
    ).replace(/^~/, homedir());
...
function getModel(config: JarvisConfig): string {
    return config.agents?.defaults?.model || "deepseek-v4-flash";
}
...
function loadRuntimeConfig(
    ...
    if (configPath) {
        ...
        if (!existsSync(resolved)) {
            console.error(
                chalk.red(`Error: Config file not found: ${resolved}`),
            );
            process.exit(1);
        }
...
export async function cmdAgent(opts: {
    ...
    if (opts.message) {
        ...
        const renderer = new StreamRenderer({
            renderMarkdown: opts.markdown ?? true,
        });
        ...
        const response = await loop.processDirect(opts.message, {
            ...
            callbacks: {
                ...
                onStreamEnd: async (resuming: boolean) =>
                    renderer.onEnd({ resuming }),
                onProgress: async (
                    content: string,
                    extra?: { toolHint?: boolean },
                ) => {
    ...
    // Interactive mode
    console.log(
        `${chalk.cyan("jarvis")} Interactive mode ${chalk.blue(`(${model})`)} — type ${chalk.bold("exit")} or Ctrl+C to quit`,
    );
    console.log(
        `  ${chalk.dim("Commands: /help /new /stop /status /dream /dream-log /dream-restore /skills /restart")}`,
    );
    console.log();
    ...
    const cliCommands = loop.commands
        .getAllCommands()
        .map((c) => c.toLowerCase());
    const rl = createInterface({
        ...
        completer: (line: string) => {
            ...
            const hits = cliCommands.filter((c) =>
...
export async function cmdServe(opts: {
    ...
    console.log(
        `  ${chalk.cyan("Endpoint")} : http://${host}:${port}/v1/chat/completions`,
    );
...
export async function cmdGateway(opts: {
    ...
    // Gateway dispatch loop: consume inbound → AgentLoop → publish outbound
    const abortController = new AbortController();
    void (async () => {
        while (!abortController.signal.aborted) {
            try {
                const msg = await bus.consumeInbound();
                const result = await loop.processMessage(msg);
                if (result) {
    ...
    // Bridge cron jobs → AgentLoop → MessageBus
    cron.onJob = async (job) => {
        const channel = job.payload.channel ?? "cron";
        const chatId = job.payload.to ?? "";
        const deliver = job.payload.deliver !== false;

        // Set tool context so cron tool can route delivery
        loop._set_tool_context(channel, chatId);
        const cronTool = loop.tools.get("cron");
        if (cronTool && "setCronContext" in cronTool) {
            (
                cronTool as { setCronContext: (active: boolean) => void }
            ).setCronContext(true);
        }
        ...
        console.log(
            `[gateway] Cron job '${job.name}' fired, dispatching to agent`,
        );
        try {
            const result = await loop.processMessage(inbound);
            if (result && deliver) {
                bus.publishOutbound(result);
            }
        } finally {
    ...
    console.log(
        `  ${chalk.green("✓")} Channels: ${channels.channels.size} enabled`,
    );
...
function mergeMissingDefaults(
    ...
    for (const [key, value] of Object.entries(defaults)) {
        if (!(key in merged)) {
            ...
            merged[key] = mergeMissingDefaults(
...
async function onboardPlugins(configPath: string): Promise<void> {
    try {
        ...
        for (const [name] of Object.entries(allChannels)) {
            ...
            const existing = channels[name] as
                | Record<string, unknown>
                | undefined;
            if (!existing) {
...
export async function cmdOnboard(opts: {
    ...
    const configPath = opts.config ? resolve(opts.config) : getConfigPath();
    ...
    if (existsSync(configPath)) {
        if (opts.wizard) {
            ...
            console.log(
                `  ${chalk.bold("y")} = overwrite with defaults (existing values will be lost)`,
            );
            console.log(
                `  ${chalk.bold("N")} = refresh config, keeping existing values and adding new fields`,
            );
            ...
            rl.question(
                chalk.blue("Overwrite? (y/N) "),
                async (answer: string) => {
                    const yes = answer.trim().toLowerCase() === "y";
                    if (yes) {
                        const config = applyWorkspaceOverride(
                            DEFAULTS as JarvisConfig,
                        );
                        saveConfig(config, configPath);
                        console.log(
                            `${chalk.green("✓")} Config reset to defaults at ${configPath}`,
                        );
                    } else {
                        const config = applyWorkspaceOverride(
                        ...
                        saveConfig(config, configPath);
                        console.log(
                            `${chalk.green("✓")} Config refreshed at ${configPath} (existing values preserved)`,
                        );
                    }
                    rl.close();
                    await finishOnboard();
    ...
    if (opts.wizard) {
        (async () => {
            try {
                ...
                console.log(
                    `${chalk.green("✓")} Config saved at ${configPath}`,
                );
            } catch (e: any) {
                if (e.message?.includes("@clack")) {
                    console.error(
                        chalk.yellow(
                            "Interactive wizard requires @clack/prompts.",
                        ),
                    );
                    return;
                }
                console.error(
                    chalk.red(`Error during configuration: ${e.message}`),
                );
                console.error(
                    chalk.yellow(
                        "Please run 'jarvis onboard' again to complete setup.",
                    ),
                );
                return;
            }
...
export async function cmdStatus(opts: { config?: string }): Promise<void> {
    ...
    console.log();
    console.log(
        `  Config:   ${configPath} ${existsSync(configPath) ? chalk.green("✓") : chalk.red("✗")}`,
    );
    console.log(
        `  Workspace: ${ws} ${existsSync(ws) ? chalk.green("✓") : chalk.red("✗")}`,
    );
    ...
    if (existsSync(configPath)) {
        for (const spec of PROVIDERS) {
            ...
            if (spec.isOAuth) {
                ...
                if (p.apiBase) {
                    console.log(
                        `  ${providerName}: ${chalk.green("✓")} ${p.apiBase}`,
                    );
                } else {
                ...
                console.log(
                    `  ${providerName}: ${hasKey ? chalk.green("✓") : chalk.dim("not set")}`,
                );
            }
        }
    }
...
export async function cmdChannelsStatus(opts: {
    config?: string;
}): Promise<void> {
...
export async function cmdChannelsLogin(
    ...
    try {
        const { discoverAll } = await import("../channels/registry");
        const allChannels: Record<string, new (...args: any[]) => any> =
            await discoverAll();
...
export async function cmdPluginsList(opts: { config?: string }): Promise<void> {
    ...
    try {
        const { discoverAll, discoverChannelNames } =
...
export async function cmdProviderLogin(providerName: string): Promise<void> {
    ...
    const spec = PROVIDERS.find(
        (s: ProviderSpec) => s.name === key && s.isOAuth,
    );
    if (!spec) {
        const oauthNames = PROVIDERS.filter((s: ProviderSpec) => s.isOAuth).map(
            (s: ProviderSpec) => s.name.replace(/_/g, "-"),
        );
    ...
    if (spec.name === "openai_codex") {
        ...
        console.error(
            "Set OPENAI_API_KEY env var or configure in config file.",
        );
    } else if (spec.name === "github_copilot") {
        console.error(
            chalk.yellow("GitHub Copilot login not yet implemented."),
        );
        console.error("Set GITHUB_TOKEN env var or configure in config file.");
    } else {
        console.error(
            chalk.red(`Login not implemented for ${spec.displayName}`),
        );
    }
}
